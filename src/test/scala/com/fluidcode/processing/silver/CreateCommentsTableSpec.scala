...
import com.fluidcode.processing.silver.CreateCommentsTable._
import org.apache.spark.sql.QueryTest
import org.apache.spark.sql.delta.test.DeltaExtendedSparkSession
import org.apache.spark.sql.test.SharedSparkSession

class CreateCommentsTableSpec extends QueryTest
  with SharedSparkSession
  with DeltaExtendedSparkSession {

  test("getCommentsData should select comments data from Bronze Layer" ) {
    ...
    val result = getCommentsData(sampleDf)

    val expectedResult = Seq(
      SilverComments("Graph1", 1623779105, "1382894360", "138289436000", "mehrez", "comment_text1", "benz")
    ).toDF()
